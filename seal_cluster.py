...
from utils import extract_enclosing_subgraphs, extract_enclosing_subgraphs_with_community_label
...
def main():
    ...
    model = DGCNN(num_features=(args.z_max + 1) * 2, hidden_channels=32, k=k).to(device)
    optimizer = torch.optim.Adam(params=model.parameters(), lr=args.lr)
    criterion = BCEWithLogitsLoss()

    print(model)
    with open(f'seal_cluster_training_log_{args.dataset}.txt', 'a') as file:
...
def train(model, optimizer, train_data, batch_size, criterion, device, z_max):
    ...
    for perm in tqdm(DataLoader(range(train_data.edge_label_index.size(1)), batch_size, shuffle=True)):
        ...
        sampled_data = extract_enclosing_subgraphs_with_community_label(train_data.x, train_data.edge_index, sampled_edge_label_index, 2)
        sampled_data = Batch.from_data_list(sampled_data)
        z_onehot = F.one_hot(torch.clamp(sampled_data.z, max=z_max), z_max + 1).to(device).float()
        z_prime_onehot = F.one_hot(torch.clamp(sampled_data.z_prime, max=z_max), z_max + 1).to(device).float()
        sampled_data.x = torch.cat([z_onehot, z_prime_onehot], dim=-1)    
        optimizer.zero_grad()
...
def test(model, test_data, batch_size, device, z_max):
    ...
    for perm in tqdm(DataLoader(range(test_data.edge_label_index.size(1)), batch_size)):
        ...
        batch_data = extract_enclosing_subgraphs_with_community_label(test_data.x, test_data.edge_index, edge_label_index, 2)
        batch_data = Batch.from_data_list(batch_data)
        z_onehot = F.one_hot(torch.clamp(batch_data.z, max=z_max), z_max + 1).to(device).float()
        z_prime_onehot = F.one_hot(torch.clamp(batch_data.z_prime, max=z_max), z_max + 1).to(device).float()
        batch_data.x = torch.cat([z_onehot, z_prime_onehot], dim=-1)            
